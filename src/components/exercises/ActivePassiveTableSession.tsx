          ...
          <table className="w-full min-w-max border-collapse">
            ...
            <tbody>
              ...
              {workedExampleItem && (() => {
                ...
                return (
                  <tr className="border-b border-parchment-darker/40 bg-[var(--color-primary-light)]">
                    <td className="py-2.5 pr-2">
                      <p className="font-sans text-xs text-gold leading-snug">{workedExampleItem.english}</p>
                      <span className="text-[9px] font-sans text-gold/60 uppercase tracking-wide">example</span>
                    </td>
                    <td className="py-2.5 px-2 text-center">
                      <span dir="rtl" className="arabic text-xs text-gold">{wa.sigha}</span>
                    </td>
                    <td className="py-2.5 px-2 text-center">
                      <span dir="rtl" className="arabic text-sm text-gold">{wa.active}</span>
                    </td>
                    <td className="py-2.5 px-2 text-center">
                      <span dir="rtl" className="arabic text-sm text-gold">{wa.passive}</span>
                    </td>
                  </tr>
                );
              })()}
              {scoreable.map((item, ri) => {
                ...
                return (
                  <tr key={ri} className="border-b border-parchment-darker/60 last:border-0">
                    {/* English */}
                    <td className="py-2.5 pr-2">
                    ...
                    <td className="py-2 px-1.5 text-center">
                      {activeState === 'correct' ? (
                        <span dir="rtl" className="arabic text-sm text-teal-dark">{answer.active}</span>
                      ) : (
                        <button
                          ...
                          className={`inline-flex items-center justify-center w-[60px] h-8 rounded-lg text-[11px] font-sans font-medium transition-all ${
                            activeState === 'wrong'
                              ? 'bg-[var(--color-accent-light)] border border-crimson/40 text-crimson'
                              : 'border border-dashed border-parchment-darker bg-white text-parchment-darker'
                          }`}
                        >
                          {activeState === 'wrong' ? '✗' : '—'}
                        </button>
                    ...
                    <td className="py-2 px-1.5 text-center">
                      {passiveState === 'correct' ? (
                        <span dir="rtl" className="arabic text-sm text-teal-dark">{answer.passive}</span>
                      ) : (
                        <button
